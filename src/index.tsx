...
export default class SmartFormSchemaHandler implements SchemaHandler {
    ...
    addHandler(handler: FieldHandler) {
    ...
    getReactElement(schema: Schema | FieldConfig, context: ReactSchemaHandlerContext): React.ReactElement {
        ...
        const buildField = (field: FieldConfig) =>
            this.getHandler(field).getReactElement(field, {...context, parents, handler: this})
    ...
    getYupSchema(schema: Schema | FieldConfig, context: YupSchemaHandlerContext): yup.Schema<unknown> {
        const parents = context.parents || []
        const buildField = (field: FieldConfig) =>
            this.getHandler(field).getYupSchema(field, {...context, parents, handler: this})

        if(Array.isArray(schema)) {
            const fields = schema
                .reduce((collected, field) => {
                    collected[field.name.toString()] = this.getHandler(field).getYupSchema(field, {...context, parents, handler: this});
                    return collected
